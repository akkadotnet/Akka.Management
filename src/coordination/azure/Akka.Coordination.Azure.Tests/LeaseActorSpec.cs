...
namespace Akka.Coordination.Azure.Tests
{
    ...
    public class LeaseActorSpec : LeaseActorTest
    {
            ...
            RunTest(() =>
            {
                ...
                UpdateProbe.Reply(
                    new Left<LeaseResource, LeaseResource>(
                        new LeaseResource("", CurrentVersion, CurrentTime)));
        ...
        public void ReleasedLockAcquiredWithNewVersion()
        {
            RunTest(() =>
            {
                ...
                var failedVersion = new ETag((CurrentVersionCount + 6).ToString());
                UpdateProbe.Reply(
                    new Left<LeaseResource, LeaseResource>(
                        new LeaseResource("", failedVersion, CurrentTime)));
                // Try again
        ...
        public void AllowLeaseOverwriteIfTtlExpiredInIdleState()
        {
            RunTest(() =>
            {
                ...
                LeaseProbe.Reply(new LeaseResource(
                    crashedClient,
                    CurrentVersion,
                    CurrentTime - new TimeSpan(LeaseSettings.TimeoutSettings.HeartbeatTimeout.Ticks * 2)));
    ...
    public abstract class LeaseActorTest: TestKit.Xunit2.TestKit
    {
        ...
        protected ETag CurrentVersion => new ETag(CurrentVersionCount.ToString());
        
        protected void IncrementVersion()
            => CurrentVersionCount++;

        protected DateTimeOffset CurrentTime => DateTimeOffset.UtcNow;
        ...
        protected void AcquireLease(Action<Exception?>? callback = null)
        {
            ...
            LeaseProbe.ExpectMsg(LeaseName);
            LeaseProbe.Reply(new LeaseResource("", CurrentVersion, new DateTimeOffset(1L, TimeSpan.Zero)));
            UpdateProbe.ExpectMsg((OwnerName, CurrentVersion));
            IncrementVersion();
            UpdateProbe.Reply(
                new Right<LeaseResource, LeaseResource>(
        ...
        protected void AzureApiFailureDuringRead()
        {
            var failure = new LeaseException("Failed to communicate with API server");
            UnderTest.Tell(new LeaseActor.Acquire(), Sender);
            LeaseProbe.ExpectMsg(LeaseName);
