...
* [`Akka.Discovery.Redis`](src/discovery/redis/Akka.Discovery.Redis) - Akka.Cluster bootstrapping discovery service using Redis. Each node registers itself under a TTL key and refreshes it on a heartbeat; peers discover one another by scanning those keys, and dead nodes are removed automatically by Redis key expiry. A lightweight alternative to `Akka.Discovery.Azure`, well suited to .NET Aspire local development and any environment that already runs Redis. You can read more in the documentation [here](https://github.com/akkadotnet/Akka.Management/blob/dev/src/discovery/redis/Akka.Discovery.Redis/README.md).
...
* [`Akka.Aspire.Hosting`](src/aspire/Akka.Aspire.Hosting) - .NET Aspire **hosting** integration. Declare an Akka.NET cluster's topology (discovery backend, replica count, ports) in your Aspire AppHost; every service replica wired with `WithReference` then discovers its peers, forms a cluster, and reports health. You can read more in the documentation [here](https://github.com/akkadotnet/Akka.Management/blob/dev/src/aspire/Akka.Aspire.Hosting/README.md).
* [`Akka.Aspire`](src/aspire/Akka.Aspire) - .NET Aspire **service** integration. `WithAspireClusterBootstrap` configures Akka.Remote, Akka.Cluster, Akka.Management, Cluster Bootstrap, and a cluster-membership health check from the configuration the `Akka.Aspire.Hosting` AppHost injects — no manual HOCON required. You can read more in the documentation [here](https://github.com/akkadotnet/Akka.Management/blob/dev/src/aspire/Akka.Aspire/README.md).
